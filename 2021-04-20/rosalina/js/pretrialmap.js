...
var div = d3.select("#pretrial_map").append("div")	
  .attr("class", "tooltip")				
  .style("opacity", 0);

var svg = d3.select("#pretrial_map").append("svg")
  .attr("width", width)
  .attr("height", height);

var ext_color_domain = [0, 1, 150, 300, 500, 750, 1000]
var legend_labels = ["No data", "<150", "<300", "<500", "<750", "<1000", "1000+"]
var color = d3.scaleThreshold()
  .domain([1, 150, 300, 500, 750, 1000])
  .range(["#ddd8d8", "#c6dbef", "#9ecae1", "#6baed6", "#2171b5", "#08519c", "#183d75"]);
...
svg.append("g")
  ...
  .on("mouseover", function(d) {		
      ...
      .style("opacity", 1)		
          div	.text(nameById[d.id] + ": "  + parseFloat(rateById[d.id]).toFixed(0))	
...
var ls_w = 80, ls_h = 15;
  ...
  legend.append("rect")
   .attr("x", function(d, i){ return width - (i*ls_w) - ls_w - 80;})
   .attr("y", 560)
   .attr("width", ls_w)
   .attr("height", ls_h)
